...
import { DeepSeekProfileForm, useAIProfiles } from "../features/ai-valuation";
...
export function AutomationPage({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  const strictProfiles = useAIProfiles({ onUnauthorized });
  ...
  async function reloadAll() { await Promise.all([strictProfiles.refresh(), usage.reload(), jobs.reload(), rules.reload(), runs.reload(), bulkAudits.reload()]); }

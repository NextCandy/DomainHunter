...
export function AutomationPage({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  const profiles = useAsync(() => api.p1.ai.providers.list(), [], onUnauthorized);
  ...
  const bulkAudits = useAsync(() => api.p1.bulkAudits(), [], onUnauthorized);
  ...
  async function reloadAll() { await Promise.all([settings.reload(), profiles.reload(), models.reload(), usage.reload(), jobs.reload(), rules.reload(), runs.reload(), bulkAudits.reload()]); }

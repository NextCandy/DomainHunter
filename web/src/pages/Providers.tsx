...
export function ProvidersPage({ onUnauthorized }: { onUnauthorized: () => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {data && <>
        <QuerySourceAlert providers={providers} />
        ...
        <div id="provider-health" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
...
function QuerySourceAlert({ providers }: { providers: ProviderHealth[] }) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-card border border-danger/35 bg-danger/8 px-4 py-3 text-[12px] text-ink" role="alert">
      <span className="min-w-0 flex-1"><strong className="text-danger">查询源告警：</strong>{" "}{atRisk.map((provider) => `${providerLabel(provider.provider)}（${provider.state === "offline" ? "离线" : `错误率 ${((provider.error_rate ?? 0) * 100).toFixed(1)}%`}）`).join("、")}</span>
      <a href="#provider-health" className="shrink-0 font-medium text-danger underline underline-offset-2">查看源状态</a>
    </div>
  );
}

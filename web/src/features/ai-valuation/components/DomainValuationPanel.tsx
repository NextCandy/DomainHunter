...
export function DomainValuationPanel({
  ...
  const { job, loading, submitting, error, enqueue, cancel, refresh, clearError } = useDomainValuation(domain, {
    ...
    onCompleted: (completed) => {
      ...
      else if (completed.state === "failed") toast("AI 估价未完成，请查看失败原因", "error");
      onCompleted?.();
    },
  });

  const hasReviewConstraint = reviewRequired || confidence === "low" || status === "error" || status === "unknown" || status === "skipped";
  const canEnqueue = !hasReviewConstraint && (!job || ["idle", "failed", "cancelled", "deferred"].includes(job.state));
  ...
  async function startValuation(forceRefresh = false) {
    ...
    if (created) toast(created.cached ? "已命中可用的估价缓存" : "已加入 AI 估价队列", "success");
  }
...
function EmptyValuationState({ disabled, reviewRequired, onStart }: { disabled: boolean; reviewRequired: boolean; onStart: () => void }) {
  return (
    <div className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end">
      ...
      {reviewRequired && <p className="sm:col-span-2 text-[11px] text-ink-muted">该操作已因数据复核状态安全禁用。</p>}
    </div>
  );
}
...
function ValuationJobContent({
  ...
  if (job.state === "succeeded" && job.result) {
    return <ValuationResultView valuation={job.result} compact={compact} showDetails={showDetails} onToggleDetails={onToggleDetails} onRefresh={() => void onStart(true)} pending={pending} cached={Boolean(job.cached)} />;
  }

  return <EmptyValuationState disabled={pending || !allowRetry} reviewRequired={!allowRetry} onStart={() => void onStart(false)} />;
}

function InProgressState({ job, pending, onCancel }: { job: ValuationJob; pending: boolean; onCancel: () => Promise<void> }) {
...
function ScoreCell({ label, score }: { label: string; score: number }) {
  return <div className="px-2"><div className="mono text-[11px] text-ink-muted">{label.toUpperCase()}</div><div className="mt-1 text-[24px] font-semibold text-ink">{score}<span className="ml-0.5 text-[11px] font-normal text-ink-muted">/100</span></div></div>;
}
...
function InsightList({ title, values, tone, empty }: { title: string; values: string[]; tone: string; empty: string }) {
  return <div><p className={cx("text-[12px] font-semibold", tone)}>{title}</p><ul className="mt-1.5 space-y-1 text-[12px] leading-5 text-ink-muted">{values.length > 0 ? values.slice(0, 3).map((value) => <li key={value}>— {value}</li>) : <li>— {empty}</li>}</ul></div>;
}

function LoadingSkeleton() {
  return <div className="animate-pulse space-y-3"><div className="h-4 w-32 bg-surface-muted" /><div className="h-7 w-56 bg-surface-muted" /><div className="h-3 w-full bg-surface-muted" /><div className="h-3 w-4/5 bg-surface-muted" /></div>;
}

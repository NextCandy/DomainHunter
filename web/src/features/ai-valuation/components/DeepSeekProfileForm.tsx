...
function validateBaseURL(baseURL: string): string | null {
  try {
    ...
    if (url.protocol !== "https:")
      return "生产环境只接受 HTTPS Base URL；本地开发例外应由后端环境变量控制。";
    if (url.pathname.replace(/\/$/, "").endsWith("/chat/completions"))
      return "请填写 Base URL，不要包含 /chat/completions；后端会自动拼接该路径。";
    if (url.username || url.password || url.search || url.hash)
      return "Base URL 不能包含账号、参数或锚点。";
    return null;
  } catch {
...
export function DeepSeekProfileForm({
  profile,
  onUnauthorized,
  onSaved,
  onCancel,
}: DeepSeekProfileFormProps) {
  ...
  function update<K extends keyof AIProfileInput>(
    key: K,
    value: AIProfileInput[K],
  ) {
    ...
    <section className="overflow-hidden rounded-card border border-line bg-surface">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-[16px] font-semibold text-ink">
              {profile ? "编辑 AI 估价档案" : "新建 AI 估价档案"}
            </h2>
            <p className="mt-1 text-[12px] leading-5 text-ink-muted">
              默认使用 DeepSeek 官方 OpenAI-compatible
              接口；密钥从不由读取接口回显，留空表示保留已保存的密钥或继续使用部署环境变量。
            </p>
          </div>
          {profile && (
            <Pill>
              {profile.api_key_set
                ? `Key：${profile.api_key_source === "environment" ? "环境变量" : "已加密保存"}`
                : "尚未设置 Key"}
            </Pill>
          )}
        </div>
      ...
      <div className="space-y-5 p-5">
        {(localError || controller.error) && (
          <ErrorNotice message={localError || controller.error || "配置错误"} />
        )}

        <section aria-labelledby="ai-connection-heading">
          <SectionHeading
            id="ai-connection-heading"
            title="连接"
            hint="服务端出站连接与档案身份"
          />
          <div className="mt-3 grid gap-4 md:grid-cols-2">
            <Field label="档案名称" hint="用于详情页和批量入队时识别。">
              <input
                className="input"
                value={form.name}
                maxLength={80}
            ...
            <Field label="提供商" hint="默认配置使用 OpenAI-compatible 协议。">
              <select
                className="input"
                value={form.provider}
                onChange={(event) =>
                  update(
                    "provider",
                    event.target.value as AIProfileInput["provider"],
                  )
                }
              >
            ...
            <Field
              label="Base URL"
              hint="填写服务根地址；不要填写 /chat/completions。"
            >
              <input
                ...
                inputMode="url"
                onChange={(event) =>
                  update("base_url", event.target.value.trim())
                }
                aria-invalid={Boolean(baseURLError)}
              />
              {baseURLError && (
                <p className="mt-1 text-[11px] text-danger">{baseURLError}</p>
              )}
            </Field>
          </div>
        </section>
        ...
        <section aria-labelledby="ai-model-heading">
          <SectionHeading
            id="ai-model-heading"
            title="模型"
            hint="模型选择只影响研究性排序，不会改变域名状态或可注册结论"
          />
          <div className="mt-3 grid gap-4 md:grid-cols-3">
            <Field
              label="模型"
              hint="批量排序建议使用快速模型；少量重点域名再选择深度模型。"
            >
              <input
                className="input mono"
                list="domainhunter-models"
                value={form.model}
                spellCheck={false}
                onChange={(event) => update("model", event.target.value.trim())}
              />
              <datalist id="domainhunter-models">
            ...
            <Field label="Thinking">
              <select
                className="input"
                value={form.thinking_type}
                onChange={(event) =>
                  update(
                    "thinking_type",
                    event.target.value as AIProfileInput["thinking_type"],
                  )
                }
              >
                <option value="disabled">关闭（默认批量）</option>
                <option value="enabled">启用（重点域名）</option>
              </select>
            </Field>
            <Field label="推理强度">
              <select
                ...
                onChange={(event) =>
                  update(
                    "reasoning_effort",
                    event.target.value as AIProfileInput["reasoning_effort"],
                  )
                }
              >
        ...
        <section aria-labelledby="ai-cost-heading">
          <SectionHeading
            id="ai-cost-heading"
            title="成本与执行"
            hint="不设置每日估价额度；仍保留超时、并发、缓存和上游限流保护"
          />
          <div className="mt-3 grid gap-4 md:grid-cols-3">
            <NumberField
              label="超时"
          ...
          <div className="mt-4 max-w-[220px]">
            <Field label="缓存 TTL">
              <select
                ...
                onChange={(event) =>
                  update("cache_ttl_hours", Number(event.target.value))
                }
              >
        ...
        <section aria-labelledby="ai-security-heading">
          <SectionHeading
            id="ai-security-heading"
            title="安全"
            hint="密钥只写不读；所有 Provider 请求由后端发起并执行 allowlist 与 SSRF 校验"
          />
          <div className="mt-3 border-y border-line py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-[12px] leading-5 text-ink-muted">
                  优先使用 <code className="mono">DOMAINHUNTER_AI_API_KEY</code>
                  。若已配置应用级加密，可在此处安全写入 Key。
                </p>
              </div>
              <Pill title="任何 GET 接口均不返回完整 Key">只写不读</Pill>
            </div>
            <input
          ...
          <label className="mt-3 flex cursor-pointer items-start gap-3">
            ...
            <span>
              ...
              <span className="mt-1 block text-[12px] leading-5 text-ink-muted">
                详情页“加入估价队列”会优先使用此档案；不覆盖已有任务的档案选择。
              </span>
            </span>
          </label>
        </section>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t border-line pt-4">
          <div>{testResult && <TestResult result={testResult} />}</div>
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              className="btn h-9"
              disabled={controller.saving || Boolean(baseURLError)}
              onClick={() => void testConnection()}
            >
              {controller.saving ? <Spinner /> : "测试连接"}
            </button>
            {onCancel && (
              <button
                type="button"
            ...
            <button
              ...
              onClick={() => void save()}
            >
              {controller.saving ? <Spinner /> : "保存档案"}
            </button>
          </div>
        </div>
      </div>
...
function SectionHeading({
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-[11px] leading-4 text-ink-muted">{hint}</p>
    </div>
  );
}
...
function TestResult({ result }: { result: ConnectionTestResult }) {
  return (
    <div
      className={
        result.ok
          ? "rounded-card border border-accent/30 bg-accent-soft/45 px-3 py-2 text-[12px] text-ink"
          : "rounded-card border border-line bg-surface-muted px-3 py-2 text-[12px] text-ink-muted"
      }
    >
